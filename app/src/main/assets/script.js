...
YouTubeCodecs.video.forEach((x)=>{
localStorage.setItem(x,"true");
});

YouTubeCodecs.audio.forEach((x)=>{
localStorage.setItem(x,"true");
});
...
/*Get Codecs*/
function getYouTubeCodecs(){
var t=`<p style="text-align:center;font-size:14px;">This feature is experimental , this may break YouTube if not configured correctly. By default all the codecs are enabled , tap on the buttons below to switch them.</p><br> <vc  style="font-size:14px;">Video Codecs</vc><br>`;

for(var y in YouTubeCodecs.video){

var x=YouTubeCodecs.video[y];
...
for(var y in YouTubeCodecs.audio){

var x=YouTubeCodecs.audio[y];
...
/*The settings tab*/
async function youtubeSettings(){
var ytpSet=document.createElement("div");
var ytpSetI=document.createElement("div");
ytpSet.setAttribute("id","settingsyoutubediv");
ytpSetI.setAttribute("id","ssyoutubedivI");
...
#settingsyoutubediv a{text-decoration:underline;} #settingsyoutubediv li{list-style:none; display:flex;align-items:center;justify-content:center;color:#fff;border-radius:25px;padding:10px;background:#000;margin:5px;}
#ssyoutubedivI div{
height:10px;
width:calc(100% - 20px);
padding:10px;
...
#ssyoutubedivI div input::placeholder{color:${ isD ? "white" : "#000"};}
#ssyoutubedivI div input,#ssyoutubedivI div button{
height:35px;
...
#ssyoutubedivI button svg{
float:right;
}
#ssyoutubedivI .credit{
...
#ssyoutubedivI .geminiModels,#ssyoutubedivI .disableCodecs,#ssyoutubedivI .geminiPrompt{
height:auto;
min-height:100px;
padding-bottom:12px;
background:${isD ? "#212121" : "#f1f1f1"};
...
#ssyoutubedivI .geminiModels:before,#ssyoutubedivI .disableCodecs:before,#ssyoutubedivI .geminiPrompt:before{
height:100%;
width:100%;
...
#ssyoutubedivI .geminiPrompt textarea{
height:300px;
width:95%;
border-radius:20px;
padding:15px;
...
#ssyoutubedivI .disableCodecs{
column:50%;
}
#ssyoutubedivI .disableCodecs button{
width:48%;
column:50%;
margin-right:2%;
color:${c};
...
ytpSetI.innerHTML+=`<br><b style='font-size:18px' >YouTube Settings</b>
<span style="font-size:10px">v${YouTubeVer}</span>
<br><br>


<div><input type="url" placeholder="Enter Youtube URL" id="youtubeUrlInput" ></div>
<br>
...
<z style="margin-left:6px">by Lifer_Lighdow</z>
</div>
`;
...
document.getElementById("youtubeUrlInput").addEventListener("keyup",searchUrl);
...
var actionsList={
  ...
  issues:()=>{
    Android.oplink('https://github.com/prateek-chaubey/YouTube/issues');
  },
  disableCodecs:()=>{
    document.getElementsByClassName('disableCodecs')[0].style.display='block';document.getElementsByClassName('disableCodecs')[0].innerHTML=getYouTubeCodecs();
  },
  sponsor:()=>{
...
try{document.getElementById("settingsyoutubediv").remove();}catch{}
a.click();
...
/*Video Downloader*/
async function youtubeDownVid(){

window.youtubeSabrDownload();
...
insertAfter(document.getElementById('youtubeMainDivE'),GeminiRes);
...
if(document.getElementById("youtubeMainDivE") == null){



var youtubeMainDivA=document.createElement("div");
youtubeMainDivA.setAttribute("id","youtubeMainDivE");
youtubeMainDivA.setAttribute("style",`
height:50px;width:100%;display:block;overflow:auto;
`);

insertAfter(document.getElementsByClassName('slim-video-action-bar-actions')[0],youtubeMainDivA);

var youtubeMainDiv=document.createElement("div");
youtubeMainDiv.setAttribute("style",`
...
`);
youtubeMainDivA.appendChild(youtubeMainDiv);

/*Gemini Button*/
var youtubeGemini=document.createElement("div");
sty(youtubeGemini);
youtubeGemini.style.width="115px";
youtubeGemini.style.height="calc(65% - 4.5px)";
youtubeGemini.style.position="relative";
youtubeGemini.style.background=`linear-gradient(${isD ? "#272727,#272727" : "#f2f2f2,#f2f2f2"}) padding-box , linear-gradient(16deg ,#4285f4 ,#9b72cb ,#d96570) border-box`;
youtubeGemini.style.border="2px solid transparent";
youtubeGemini.innerHTML=`
...
youtubeMainDiv.appendChild(youtubeGemini);


youtubeGemini.addEventListener("click",
async function(){


if(parseFloat(Android.getInfo()) < parseFloat(YouTubeVer)){
/* 自動更新已移除 */
...
youtubeMainDiv.appendChild(youtubeFavElem);
youtubeFavElem.addEventListener("click",()=>{youtubeHeart(youtubeFavElem);});



/*Download Button*/
var youtubeDownVidElem=document.createElement("div");
sty(youtubeDownVidElem);
youtubeDownVidElem.style.width="140px";
youtubeDownVidElem.innerHTML=`${downBtn.replace('width="18"','width="24"').replace('height="18"','height="24"')}<span style="margin-left:2px">Download<span>`;
youtubeMainDiv.appendChild(youtubeDownVidElem);
youtubeDownVidElem.addEventListener("click",
function(){
window.location.hash="download";
});

/*PIP Button*/
var youtubePIPVidElem=document.createElement("div");
sty(youtubePIPVidElem);
youtubePIPVidElem.style.width="140px";
youtubePIPVidElem.innerHTML=`<svg xmlns="http://www.w3.org/2000/svg" height="22" viewBox="0 0 24 24" width="22"><path fill="${c}" d="M18 7h-6c-.55 0-1 .45-1 1v4c0 .55.45 1 1 1h6c.55 0 1-.45 1-1V8c0-.55-.45-1-1-1zm3-4H3c-1.1 0-2 .9-2 2v14c0 1.1.9 1.98 2 1.98h18c1.1 0 2-.88 2-1.98V5c0-1.1-.9-2-2-2zm-1 16.01H4c-.55 0-1-.45-1-1V5.98c0-.55.45-1 1-1h16c.55 0 1 .45 1 1v12.03c0 .55-.45 1-1 1z"/></svg><span style="margin-left:8px">PIP Mode<span>`;
youtubeMainDiv.appendChild(youtubePIPVidElem);
youtubePIPVidElem.addEventListener("click",
function(){
PIPlayer(true);
});
...
if(document.getElementById("youtubeMainSDivE") == null){
var ys=document.createElement("div");
ys.setAttribute("id","youtubeMainSDivE");
...
var youtubeH=document.createElement("div");
var youtubeHh=document.createElement("div");
youtubeHh.setAttribute("id","heartyoutubediv");
youtubeH.setAttribute("id","outerheartsdiv");
youtubeH.setAttribute("style",`
height:100%;width:100%;position:fixed;top:0;left:0;
display:flex;justify-content:center;
...
`);
youtubeHh.innerHTML=`<style>#heartyoutubediv a{text-decoration:none;} #heartyoutubediv li{list-style:none; display:flex;align-items:center;border-radius:15px;padding:0px;background:${d};margin:5px;}</style>`;
youtubeHh.innerHTML+="Liked Videos<ul id='listurl'>";


youtubeHh.innerHTML+="<style>.thum{height:70px;border-radius:5px;}.thum img{float:left;height:70px;width:125px;border-radius:15px 0 0 15px;flex-shrink: 0;}</style>";

document.body.appendChild(youtubeH);
youtubeH.appendChild(youtubeHh);

youtubeH.addEventListener("click",
function(ev){
if(!event.composedPath().includes(youtubeHh)){
history.back();
}
});
...
youtubeHh.innerHTML+="No Videos Found";
}else{

var v=JSON.parse(localStorage.getItem("hearts"));

if(Object.keys(v).length === 0){
return youtubeHh.innerHTML+="No Videos Found";
}

for(var n=Object.keys(v).length - 1; n >  -1 ; n--){
var x=Object.keys(v)[n];
youtubeHh.innerHTML+=`<li class="thum" >
...
youtubeHh.addEventListener("click",(e)=>{
  var el=e.target.closest("[data-action]");
...
window.onhashchange=()=>{
try{document.getElementById("outerdownyoutubediv").remove();}catch{}
try{document.getElementById("outerheartsdiv").remove();}catch{}
try{document.getElementById("settingsyoutubediv").remove();}catch{}
//try{document.querySelector("#youtubeDownloadIndicator").remove();}catch{}
//try{document.querySelector("#youtubeDownloaderDiv").remove();}catch{}
...
Latest Version ${YouTubeVer} of YouTube is available , update the YouTube to get latest features.
...
x.addEventListener("click",(e)=>{
  ...
  if(action == "url"){
    Android.oplink('https://github.com/prateek-chaubey/YouTube/releases');
  }else if(action == "download"){
    Android.downvid('YouTube.zip','https://nightly.link/prateek-chaubey/YouTube/workflows/gradle/main/YouTube.zip','application/zip');
