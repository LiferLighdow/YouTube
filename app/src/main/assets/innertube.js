...
window.youtubeSabrDownload= async function() {


var youtubeDownDiv=getDownloadElement();

youtubeDownDiv.querySelector("#videoViewDiv").innerHTML="Loading...";
...
console.error('[YouTube] PoToken generation failed:', e);
return null;
}
}
...
youtubeDownDiv.insertAdjacentHTML('beforeend',`<style>#downyoutubediv a{text-decoration:none;} #downyoutubediv li{list-style:none; display:flex;align-items:center;justify-content:center;border-radius:25px;padding:8px;background:${d};margin:5px;margin-top:8px}
#downyoutubediv select {
min-height: 20px;
width: auto;
border-radius: 25px;
border: 0;
...
youtubeDownDiv.querySelector("#videoViewDiv").innerHTML=`<label for="selectLang" style="margin-right:5px;">Language:</label>`;
...
youtubeDownDiv.querySelector("#videoViewDiv").appendChild(langList);
...
youtubeDownDiv.querySelector("#videoViewDiv").appendChild(div);
return div;
}
...
var div=youtubeDownDiv.querySelector("#thumbViewDiv");
...
div.innerHTML+=`<li class="thu" data-url="${thumb.url}" data-title="Thumbnail ${thumb.height} &#x2715; ${thumb.width} ${safeTitle} YouTube.jpg" >
...
var div=youtubeDownDiv.querySelector("#captionsViewDiv");
...
var t=`Captions ${safeTitle} YouTube`;
...
youtubeDownDiv.querySelector("#videoViewDiv").innerHTML="Downloading live streams <br>aren't supported at the moment";
}else{
updateMuxFormats();
...
var downloaderDiv=document.querySelector("#youtubeDownloaderDiv");
...
elDetails.className="youtubeDetails";
elProgressBar.className="youtubeProgressBar";
elProgress.className="youtubeProgress";
...
const youtubeDown = isExisting("outerdownyoutubediv") || document.createElement("div");
const youtubeDownDiv = isExisting("downyoutubediv") || document.createElement("div");

youtubeDown.id = "outerdownyoutubediv";
youtubeDownDiv.id = "downyoutubediv";

Object.assign(youtubeDown.style, {
height: "100%", width: "100%", position: "fixed",
top: "0", left: "0", display: "flex",
justifyContent: "center", background: "rgba(0,0,0,0.4)", zIndex: "9"
});

Object.assign(youtubeDownDiv.style, {
...
youtubeDown.addEventListener("click", (ev) => {
if (!youtubeDownDiv.contains(ev.target)) history.back();
});
...
youtubeDownDiv.appendChild(view);
views.push(view);
});
...
document.body.appendChild(youtubeDown);
youtubeDown.appendChild(youtubeDownDiv);
youtubeDownDiv.prepend(tabs); // tabs sit above views

tabs.children[0].style.background=d;
document.querySelector("#videoViewDiv").style.display = "block"

return youtubeDownDiv;
}
...
if (!filePort) {
console.error(`[YouTube] Failed to get port for ${fileName}`);
return 0;
}
...
if(document.querySelector("#youtubeDownloaderDiv")) return;

var div=document.createElement("div");

div.id="youtubeDownloaderDiv";
...
<br>
<span style="opacity:0.8;"> INFO: Do NOT close YouTube while we are downloading the files<br>
...
if(document.querySelector("#youtubeDownloadIndicator") ) return;
var div=document.createElement("div");
div.id="youtubeDownloadIndicator";
...
div.addEventListener("click",(e)=>{
var el=document.querySelector("#youtubeDownloaderDiv");
