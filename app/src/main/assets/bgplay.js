...
var youtubeAud = document.getElementsByClassName('video-stream')[0];

if (value === 'playing') {
setTimeout(()=>{Android.bgPlay(youtubeAud.currentTime*1000);},100);
} else if (value === 'paused' && (pauseAllowed || PIPause)) {
setTimeout(()=>{Android.bgPause(youtubeAud.currentTime*1000);},100);
...
setTimeout(()=>{Android.bgUpdate(iconBase64.replace("data:image/png;base64,", ""),info.title,info.artist,youtubeAud.duration*1000);},50);
setTimeout(()=>{Android.bgPlay(youtubeAud.currentTime*1000);},100);
}
else{
window.serviceRunning=true;
setTimeout(()=>{Android.bgStart(iconBase64.replace("data:image/png;base64,", ""),info.title,info.artist,youtubeAud.duration*1000);},50);
setTimeout(()=>{Android.bgPlay(youtubeAud.currentTime*1000);},100);
}
